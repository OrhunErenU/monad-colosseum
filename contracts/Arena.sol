...
contract Arena is AccessControl, ReentrancyGuard {
    ...
    /// @notice Arena tier levels with different entry fees and capacities
    enum Tier {
        BRONZE,     // 0 — 1 MON entry, max 8 players
        SILVER,     // 1 — 10 MON entry, max 6 players
        GOLD,       // 2 — 100 MON entry, max 4 players
        PLATINUM,   // 3 — 50 MON entry, max 4 players
        DIAMOND     // 4 — 250 MON entry, max 2 players
    }
    ...
    /// @notice Mapping from participant address => agentId in registry
    mapping(address => uint256) public participantAgentId;

    /// @notice Tier entry fee configuration (in wei)
    mapping(Tier => uint256) public tierEntryFee;

    /// @notice Tier max participants
    mapping(Tier => uint256) public tierMaxParticipants;

    /// @notice Arena counter
    uint256 public nextArenaId;

    /// @notice Platform fee (5%)
    uint256 public constant PLATFORM_FEE_BPS = 500;

    /// @notice External agent reward cut (50% to platform)
    uint256 public constant EXTERNAL_WINNER_CUT_BPS = 5000;
    ...
    constructor(
        ...
        leaderboard = Leaderboard(payable(_leaderboard));

        // Initialize tier entry fees (in ether/MON)
        tierEntryFee[Tier.BRONZE]   = 1 ether;
        tierEntryFee[Tier.SILVER]   = 10 ether;
        tierEntryFee[Tier.GOLD]     = 100 ether;
        tierEntryFee[Tier.PLATINUM] = 50 ether;
        tierEntryFee[Tier.DIAMOND]  = 250 ether;

        // Initialize tier max participants
        tierMaxParticipants[Tier.BRONZE]   = 8;
        tierMaxParticipants[Tier.SILVER]   = 6;
        tierMaxParticipants[Tier.GOLD]     = 4;
        tierMaxParticipants[Tier.PLATINUM] = 4;
        tierMaxParticipants[Tier.DIAMOND]  = 2;
    }
     ...
     * @param tier The tier level determining entry fee and max participants
     * @return arenaId The ID of the newly created arena
     */
    function createArena(Tier tier) external onlyRole(GAME_MASTER_ROLE) returns (uint256 arenaId) {
        arenaId = ++nextArenaId;
        emit ArenaCreated(arenaId, tier, tierEntryFee[tier], tierMaxParticipants[tier]);
    }

    /**
     * @notice Get tier entry fee
     * @param tier The tier to query
     * @return The entry fee in wei
     */
    function getTierEntryFee(Tier tier) external view returns (uint256) {
        return tierEntryFee[tier];
    }

    /**
     * @notice Get tier max participants
     * @param tier The tier to query
     * @return The max number of participants
     */
    function getTierMaxParticipants(Tier tier) external view returns (uint256) {
        return tierMaxParticipants[tier];
    }
        ...
        returns (uint256 roundId) 
    {
        ...
        // Map participant addresses to their registry agent IDs
        for (uint256 i = 0; i < participants.length; i++) {
            AgentRegistry.Agent memory agentData = agentRegistry.getAgentByWallet(participants[i]);
            if (agentData.owner != address(0)) {
                uint256 agentId = agentRegistry.walletToAgent(participants[i]);
                participantAgentId[participants[i]] = agentId;
            }
        }
    ...
    function _applyBuffs(uint256 roundId, address[] memory participants) internal {
        for (uint256 i = 0; i < participants.length; i++) {
            ...
            try buffOracle.getActiveBuffs(agent) returns (
                uint16 healthBuff,
                uint16 armorBuff,
                uint16 attackBuff,
                uint16 speedBuff
    ...
    function _completeRound(uint256 roundId, address winner) internal {
        ...
        // ── AgentRegistry + Leaderboard integration ──────────────
        uint256 winnerAgentId = participantAgentId[winner];

        // Update ELO & record match for every loser vs winner
        for (uint256 i = 0; i < round.participants.length; i++) {
            address participant = round.participants[i];
            if (participant == winner) continue;

            uint256 loserAgentId = participantAgentId[participant];
            if (winnerAgentId != 0 && loserAgentId != 0) {
                // Update ELO in AgentRegistry
                try agentRegistry.updateElo(winnerAgentId, loserAgentId, roundId) {} catch {}

                // Record earnings in AgentRegistry
                try agentRegistry.recordEarnings(winnerAgentId, round.prizePool) {} catch {}
                try agentRegistry.recordLoss(loserAgentId, round.prizePool / round.participants.length) {} catch {}

                // Record match in Leaderboard
                int256 winnerNewElo = agentRegistry.getAgentElo(winnerAgentId);
                int256 loserNewElo = agentRegistry.getAgentElo(loserAgentId);
                try leaderboard.recordMatch(
                    winnerAgentId,
                    loserAgentId,
                    roundId,
                    winnerNewElo,
        ...
        // Record betrayals in leaderboard
        for (uint256 i = 0; i < round.participants.length; i++) {
            address agent = round.participants[i];
            BattleAction memory action = pendingActions[agent][roundId];
            if (action.actionType == ActionType.ATTACK && action.target != address(0)) {
                uint256 agentId = participantAgentId[agent];
                if (agentId != 0) {
                    try leaderboard.recordBetrayal(agentId) {} catch {}
                }
            }
        }
    ...
    /// @notice Allow any registered agent to join an ACCEPTING round by paying entry fee
    /// @param roundId The round to join
    /// @param agentWallet The agent wallet address to participate with
    function joinRound(uint256 roundId, address agentWallet) 
        external 
        payable 
        nonReentrant 
    {
        Round storage round = rounds[roundId];
        if (round.status != RoundStatus.ACCEPTING) {
            revert RoundNotAccepting(roundId);
        }
        if (round.participants.length >= MAX_PARTICIPANTS) {
            revert TooManyParticipants();
        }
        if (_isParticipant(roundId, agentWallet)) {
            revert AgentAlreadyActed(agentWallet);
        }

        // Verify agent is registered
        uint256 agentId = agentRegistry.walletToAgent(agentWallet);
        require(agentId != 0, "Agent not registered");
        
        AgentRegistry.Agent memory agentData = agentRegistry.getAgent(agentId);
        require(agentData.owner == msg.sender || msg.sender == agentWallet, "Not agent owner");

        // Fee handling
        require(msg.value >= MIN_ENTRY_FEE, "Below minimum entry fee");

        // Split fee: 95% to prize pool, 5% platform fee
        uint256 platformFee = (msg.value * PLATFORM_FEE_BPS) / 10000;
        uint256 prizeContribution = msg.value - platformFee;

        round.participants.push(agentWallet);
        round.prizePool += prizeContribution;
        participantAgentId[agentWallet] = agentId;

        // Send platform fee to revenue distributor
        if (platformFee > 0) {
            (bool sent, ) = revenueDistributor.call{value: platformFee}("");
